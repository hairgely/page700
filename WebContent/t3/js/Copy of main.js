var webm = 0;var loginflg = false;var editmode = true;var cateData = [];var ucaten = "";var ucatentext = "";var firstFlg = true;var urlDo = "";
	...
	function getCate() {
		...
		$.post('/plagiarism/getCate'+urlDo, {
			...
			var html2 = '';
			html2 += '<li><button type="button" onclick="getSite(\'0\');" nname="내즐겨찾기" num="0" class="btn btn-default btn-sm catelistButton">MY</button></li>';
			for ( var k = 0; k < data.resultCate.length; k++) {
				var cate = data.resultCate[k];
				if(cate.stype=='0'){
					html2 += '<li><button type="button" onclick="getSite(' + cate.num + ');" class="btn btn-default btn-sm catelistButton" nname="'+cate.name+'" num="'+cate.num+'">'+cate.name+'</button></li>';
				}
			}
