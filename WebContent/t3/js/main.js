...
var webm = 0;var loginflg = false;var editmode = true;var cateData = [];var ucaten = "";var ucatentext = "";var firstFlg = true;var urlDo = "";
	$(function() {
		...
		$('#ucateaddok').click(function(){
			...
			if(title != ""){
				...
				$.post("" +rootStr+'/plagiarism/ucateadd'+urlDo, {
					mode : '',
	...
	function getCate() {
		$('#cateDis').empty();
		$('#dataDis').empty();
		$.post(rootStr+'/plagiarism/getCate'+urlDo, {
			mode : ''
		}, function(data) {
			var html = '';
	...
	function getSite(cate) {
		...
		$('.catelist').each(function(){
			...
			if(num == cate){
				...
				//$(this).css("background-color","#149c82");
			}else{
	...
	function getSiteLo(cate, stpes){
		...
		if(cate == 0 && !loginflg){
			...
			var saveiid = iid+"."+cate;
			var resultList = $('#dataTmp').data(saveiid);
			if(resultList != null && resultList != undefined && resultList.legnth != 0){
				loDisplay(resultList, cate, stpes, iid);
			}else{
				$.post(rootStr+'/plagiarism/getSite'+urlDo, {
					mode : '',
					cate : cate,
					iid : iid
				}, function(data) {
					
					loDisplay(data.resultList, cate, stpes, iid);
					$('#dataTmp').data(saveiid, data.resultList);
				}, 'json');
			}
	...
	function loDisplay(resultList, cate, stpes, iid, saveiid){
		if(stpes){
		...
		if(cate == "0" && loginflg){
			if($('#dataTmp2').val() != ""){
				html += '<button type="button" class="btn btn-primary btn-lg" id="siteuadd01" onclick="dirinput3();">사이트<br/>추가</button>';
			}
		}
		if (html != '') {
			$('#dataDis').html(html);
	...
	var iid = $('#dataTmp').data("login.id");
		if(iid == null || iid == ""){
			...
			$.post(rootStr+'/plagiarism/addfavorite'+urlDo, {
				mode : '', num : num, iid : iid
			}, function(data) {
				messagePopup("저장되었습니다.","즐겨찾기");
				var iid = $('#dataTmp').data("login.id"); ////
				var saveiid = iid+".0";
				$('#dataTmp').removeData(saveiid);
			}, 'json');
		}
	...
	function joinStart(){
		...
		if(id == ''){
		...
		}else if(id == "main" || id == "map" || id == "discount" || id == "text"
			|| id == "addiscount" || id == "mains" || id == "admain" || id == "editpwd" 
			|| id == "adtext"){
			$('#errordiv').text("사용할 수 없는 아이디 입니다.");
		}else{
			...
			if(pwd != cpwd){
				...
				$('#errordiv').text("");
				$.post(rootStr+'/plagiarism/joinStart'+urlDo, {
					mode : 'joinStart',
					id : id,
					pwd : pwd,
					joinemail : joinemail,
	...
	function loginstart(){
		...
		if(id == ''){
			...
			$('#loginerrordiv').text("");
			$.post(rootStr+'/plagiarism/loginstart'+urlDo, {
				mode : '',
				id : id,
				pwd : pwd
			}, function(data) {
	...
	function loginTmpstart(id){
		$.post(rootStr+'/plagiarism/loginTmpstart'+urlDo, {
			mode : '',
			id : id
		}, function(data) {
			if(data.result == "1"){
...
//				$(this).css("background-color","#149c82");
		...
		$('#submenu').empty();	
		var iid = $('#dataTmp').data("login.id");
		 $.post(rootStr+'/plagiarism/getSiteu'+urlDo, {
	...
	function saveConfirm (){
		...
		var iid = $('#dataTmp').data("login.id");
		if(type == "delfavorite"){
			$.post(rootStr+'/plagiarism/delfavorite'+urlDo, {
					mode : '', num : num, iid : iid
				}, function(data) {
					var iid = $('#dataTmp').data("login.id");
					var saveiid = iid+".0";
				...
				$.post(rootStr+'/plagiarism/ucatedel'+urlDo, {
					mode : 'ucatedel',
					id : $('#dataTmp').data("login.id"),
					num : num
				}, function(data) {
			...
			$.post(rootStr+'/plagiarism/delsiteu'+urlDo, {
					mode : '', num : num, id : iid
				}, function(data) {
					var iid = $('#dataTmp').data("login.id");
					var saveiid = iid+"."+ucaten;
					$('#dataTmp').removeData(saveiid);
					$('#closeConfirmModal').click();
					messagePopup("삭제 되었습니다.","즐겨찾기");
					getSiteu(ucaten);
	...
	function orderprev(num,orderb){
		...
		if(pnum != ''){
			var iid = $('#dataTmp').data("login.id");
			$.post(rootStr+'/plagiarism/orderprev'+urlDo, {
	...
	function setSite2(){
		...
		if(insitename == ""){
			...
			$.post(rootStr+'/plagiarism/inputsite2'+urlDo, {
				mode : '',
				id : id,
				name : insitename,
				addr : insiteaddr,
	...
	function setSite3(){
		...
		if(insitename == ""){
			...
			$.post(rootStr+'/plagiarism/inputsite3'+urlDo, {
				mode : '',
				id : id,
				name : insitename,
				addr : insiteaddr,
	...
	function ucateeditnameup(ucatenum,focusid,ordby){
		...
		if(pnum != "" && pordby != ""){
			$.post(rootStr+'/plagiarism/ucateeditnameup'+urlDo, {
	...
	function ucateeditOpenYn(openYn, num){
		$.post(rootStr+'/plagiarism/ucateeditOpenYn'+urlDo, {
	...
	function findPwdGo(){
		...
		if(findPwdId == ""){
			...
			$('#finderrordiv').text("");
			$.post(rootStr+'/plagiarism/sendEmailFindPwd'+urlDo, {
				mode : '',
				id : findPwdId,
				email : findPwdEmail
			}, function(data) {
