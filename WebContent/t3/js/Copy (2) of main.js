var webm = 0;var loginflg = false;var editmode = true;var cateData = [];var ucaten = "";var ucatentext = "";var firstFlg = true;var urlDo = "";
	...
	function buttonEf(){
		$('.imageClass').mousedown(function(){
			...
			setTimeout("window.open('"+goUrl+"');",100);
		});
	}
	...
	function getCate() {
		...
		$.post('/plagiarism/getCate'+urlDo, {
			...
			var html2 = '';
			html2 += '<button type="button" onclick="getSite(\'0\');" nname="내즐겨찾기" num="0" class="btn btn-default btn-sm catelistButton">내즐겨찾기</button>';
			for ( var k = 0; k < data.resultCate.length; k++) {
				var cate = data.resultCate[k];
				if(cate.stype=='0'){
					html2 += '<button type="button" onclick="getSite(' + cate.num + ');" class="btn btn-default btn-sm catelistButton" nname="'+cate.name+'" num="'+cate.num+'">'+cate.name+'</button>';
				}
			}
		...
		if(cate == 0 && !loginflg){
			...
			$.post('/plagiarism/getSite'+urlDo, {
				...
				for ( var i = 0; i < data.resultList.length; i++) {
					...
					html += '<img class="pointer imageClass img-rounded" ads="'+evt.addre+'" src="./img/site/'+evt.num+'.png" style="margin-top:5px;width:100%;height:100%;" goUrl="';
	...
	function viewucate(data){
		...
		for ( var k = 0; k < data.resultList.length; k++) {
			var cate = data.resultList[k];
			if(cate.stype=='0'){
				html2 += '<button type="button" onclick="getSiteu(' + cate.num + ');" class="btn btn-default btn-sm catelistButton mainDivd24" nname="'+cate.name+'" num="'+cate.num+'">'+cate.name+'</button>';
			}
		}
		...
		var iid = $('#dataTmp').data("login.id");
		 $.post('/plagiarism/getSiteu'+urlDo, {
			...
			for ( var i = 0; i < data.resultList.length; i++) {
				...
				html += '</div>';
				html += '<div class="panel-body">';
				html += '<img class="pointer imageClass img-rounded"src="http://img.bitpixels.com/getthumbnail?code=48861&size=200&url='+evt.addre+'" style="margin-top:5px;width:100%;height:100%;" goUrl="';
				if(webm == 0 || (evt.addrem == null || evt.addrem == "null" || evt.addrem == "")){
					html += evt.addre + '"\'/>';
				}else{
					html += evt.addrem + '"\'/>';
	...
	function setSite2(){
		...
		if(insitename == ""){
			...
			$.post('/plagiarism/inputsite2'+urlDo, {
				mode : '',
				id : id,
				name : insitename,
				addr : insiteaddr,
